...
class NewsFetcher:
    ...
    def fetch_from_newsapi(self, query="news", hours_back=24, lang='en'):
        ...
        if not self.newsapi_key:
            # Silent fail if no key, handle in UI
            return []
    ...
    def fetch_from_gnews(self, query="news"):
        ...
        try:
            ...
            # GNews uses 2-letter codes mostly, but let's ensure we pass the right one
            params = {
    ...
    def fetch_real_time_news(self, queries=None, lang='en'):
        ...
        if queries is None:
            # Default fallback query if none provided
            queries = ["technology"]
        ...
        unique = {v['title']:v for v in all_articles}.values()
        return list(unique)